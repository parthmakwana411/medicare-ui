                          ...
                          <div>
                            ...
                            <p-inputnumber
                              *ngIf="item.quantity && item.quantity > 0"
                              ...
                              [step]="1"
                              (onInput)="onQuantityChange(item, $event)"
                            >
...
export class MedicineCategory implements OnInit {
  ...
  async ngOnInit() {
    try {
      ...
      products = products.map((p) => {
        ...
        if (cartItem) p.quantity = cartItem.quantity;
        this.previousQuantities.set(p.id, p.quantity ?? 0);
        return p;
      });
  ...
  addToCart(product: Product) {
    product.quantity = 1;
    this.previousQuantities.set(product.id, 1);
  ...
  onQuantityChange(product: Product, event: any) {
    const newQty = Number(event.value);
    const prevQty = this.previousQuantities.get(product.id) ?? 0;

    // Only send if quantity changed
    if (newQty === prevQty) return;

    const change = newQty > prevQty ? +1 : -1;

    this.productService.addToCart(Number(product.id), change).subscribe({
      next: (res) => console.log(`Quantity ${change > 0 ? 'increased' : 'decreased'}:`, res),
